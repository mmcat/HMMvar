...
int main(int argc, char** argv){
	...
	ScoreVar score;
    score.SetQuerySequenceFromFastaFile(opt.query_file_name_.c_str(),opt.query_file_name_); // set query sequence from fasta file
...
// If there is a blast result file exists in advance, this step is skiiped automatically.
	...
	if(opt.blast_output_file_name_.empty())
		...
		Log("Blast file exists! Homologous sequence search is skipped\n",true);
...
// step 3: parse blast results and make multiple sequence alignment (MSA) by muscle.
    ...
    score.getScore(opt.hmmer_command_,wtaa_query_file_name,group_align_file_name);
    grouped_wtscores.push_back(score.wtscores);
    grouped_mtscores.push_back(score.mtscores);
    grouped_ids.push_back(score.ids);
